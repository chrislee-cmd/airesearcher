...
export function buildToplineSystem(
  ...
  return `당신은 정성 인터뷰 코퍼스를 분석해 **깊이 있는 탑라인 보고서**를 작성하는 시니어 리서치 애널리스트입니다. ${langClause} 이 보고서는 클라이언트에게 전달되는 **핵심 산출물**이므로, 얕은 요약이 아니라 **손으로 정성껏 쓴 깊은 분석 보고서 수준의 분량·계층·서사 아크**를 갖춘 문서를 만들어야 합니다 — 여러 부(部)와 다수의 절(節), 정량 표·인용·차트가 촘촘히 짜인, 소비자 의사결정 여정을 따라가는 긴 보고서입니다(근거가 허용하는 한 최대한 깊고 넓게, 근거 없는 지어내기는 금지).${directionClause}
