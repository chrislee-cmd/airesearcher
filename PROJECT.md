...
| 머지 시 마이그 자동 적용 | CI (`Apply migrations on merge`, `.github/workflows/apply-migrations.yml`) | **hard** | main push(=머지)에 신규 14-digit 마이그가 있으면 Management API 로 prod 자동 적용 + ledger insert. **additive 만 자동**, destructive(drop/type/rename/truncate/delete) 는 skip + 커밋 코멘트 + job red. 적용 실패 시 job red → 즉시 인지 (§7.5) |
| sync.sh 마이그 drift 감지 | sync.sh (jarvis 실행, 머지 감지 후) | **soft** | 자동 적용(위 워크플로) 의 백스톱. `supabase migration list --linked` 비교 → prod 미적용 마이그 발견 시 한 줄 알림. destructive 스킵분 등 자동 적용 안 된 잔여만 남음 (CLI 없으면 silent skip — §7.5 참고) |
...
### 7.5 Supabase 마이그레이션 — 머지 시 자동 적용 (additive), destructive 는 수동
**자동 적용 (2026-07-13 도입, `.github/workflows/apply-migrations.yml`)**: main 에 머지된 push 가 신규 14-digit 마이그를 추가하면, GitHub Action 이 **Supabase Management API 로 prod 에 자동 적용**하고 `supabase_migrations.schema_migrations` ledger 에 등록합니다. `supabase db push` 가 아니라 Management API 직접 실행이라 legacy 4-digit prefix 충돌(§7.9) 을 sidestep — parking-trick 불필요. 이게 마이그 drift 근본 원인(코드-DB 배포 분리) 의 구조적 fix 입니다 (2026-06-28 translate 데이터손실 · 2026-07-06 desk 500 재발 방지).

**안전 게이트 — additive 만 자동**: `add column` / `create table` / `create index` / `alter ... add` 류만 자동 적용. **destructive 패턴**(`drop table`, `drop column`, `alter column ... type`, `set data type`, `rename`, `truncate`, `delete from`) 감지 시 → **자동 적용 안 함** + 커밋 코멘트로 "수동 적용 필요" 경고 + workflow job **red**. 데이터 소실 위험은 사람 판단으로 남깁니다. 적용 실패(SQL 에러 등) 도 job red 라 즉시 인지됩니다. 각 SQL 은 `if not exists`/`if exists` idempotent 관례라 재실행 안전.

**Secret**: workflow 는 repo secret `SUPABASE_ACCESS_TOKEN` 를 사용 (Settings → Secrets and variables → Actions, 값 = env-ssot 동일 토큰). 미등록 시 job 이 명시적으로 fail 합니다.

**여전히 수동인 경우**: destructive 마이그(위 게이트로 스킵됨) 는 사람이 검토 후 대시보드 SQL editor 또는 `supabase db push` 로 적용. PR 머지 후 `git log --oneline -- supabase/migrations` 로 새 마이그를 확인하고, 커밋 코멘트에 destructive 스킵 경고가 있으면 즉시 처리하세요.
...
**백스톱 감지 (2026-06-28 사고 후 도입)**: 위 자동 적용의 안전망으로, jarvis 의 `sync.sh` 가 머지 감지 직후 `supabase migration list --linked` 로 prod-미적용 마이그 발견 시 한 줄 알림. 자동 적용이 정상 동작하면 destructive 스킵분 정도만 여기 잡힙니다. supabase CLI 없는 환경에서는 silent skip — soft 알림, sync.sh 막지 않음.
...
**2026-07-13 이후 — 자동 적용은 이 충돌을 우회함**: 머지 시 자동 적용 워크플로(§7.5, `apply-migrations.yml`) 는 `supabase db push` 가 아니라 **Management API 직접 실행 + 명시적 ledger insert** 를 씁니다. prefix-truncate 매칭 경로를 아예 타지 않으므로 legacy 13개가 남아 있어도 새 14-digit 마이그의 자동 적용은 막히지 않습니다. 아래 parking trick 은 **수동** `db push` 를 돌려야 할 때(로컬 복구 등) 만 필요합니다.

**Legacy 13개에 부딪힐 때 (수동 `db push` 경로) — "parking trick"**:
...
### 8.1 Auth 이메일 발송 (SSOT — 커스텀 SMTP)

**Auth 이메일(OTP·매직링크·비밀번호 재설정·이메일 변경)은 Resend 커스텀 SMTP 로 발송한다.** Supabase 내장 SMTP 는 **테스트 전용(시간당 ~2통 하드캡)** 이라 실사용·데모·공유 뷰어 OTP 에 부적합 — 캡 초과 시 GoTrue 가 발송을 **조용히 거부**하고 `auth.users.*_sent_at` 스탬프가 안 찍힌다 (2026-07-09 "인증메일이 갑자기 안 온다" 사고의 root cause).

| 항목 | 값 |
|---|---|
| SMTP 호스트 | `smtp.resend.com` (Port 465 SSL / 587 TLS) |
| SMTP User | `resend` · Pass = Resend API key |
| 발신 주소 | `no-reply@research-canvas.io` (SPF·DKIM·DMARC 인증된 도메인) |
| 이메일 rate limit | 내장 ~2/시간 → **실사용 수준(30~100/시간)으로 상향** |

- **자격/rate limit 의 라이브 SSOT 는 Supabase 대시보드** — `Authentication → Emails → SMTP Settings` (자격) 및 `Authentication → Rate Limits → "Rate limit for sending emails"` (상향값). MCP·repo 로는 편집 불가하며, **API key·DNS 시크릿은 절대 repo 에 커밋하지 않는다** (§6, 대시보드에만 존재).
- **설정/DNS 등록은 대시보드 액션** (Resend 계정 + `research-canvas.io` DNS 등록 + Supabase 자격 입력). 이 표는 "무엇이 어떻게 구성돼야 하는가"의 결정 SSOT 이고, 실 적용값 확인은 대시보드에서.
- **검증 절차** (SMTP 연결 후):
  1. 공유 뷰어 OTP 를 **연속 5회+** 요청 → 매번 도착 (내장 캡 안 걸림).
  2. 매 요청마다 스탬프가 갱신되는지 SQL 확인:
     ```sql
     select max(recovery_sent_at) last_recovery, max(confirmation_sent_at) last_confirm,
            count(*) filter (where recovery_sent_at > now() - interval '10 min') sent_10m
     from auth.users;
     ```
  3. 수신 메일 헤더에서 SPF/DKIM `pass` (또는 Resend 대시보드 `delivered`) → 스팸함 안 감.
- 관련: §7.12(단일세션 signOut 부작용), 자매 작업 발송 실패 관측성(`.catch(()=>{})` 무음화) — SMTP 는 캐파(근본 원인), 관측성은 독립 가치.
...
- Primitives는 `src/components/ui/` 에 있음. 현재 보유: **Button · IconButton · ChromeButton · Input · ChromeInput · ChipInput · Textarea · Checkbox · Select · Slider · Modal · Skeleton · Label · FileDropZone · EmptyState · BrandLoader · JobProgress · `*-Menu` 류**. 새 native `<button>/<input>/<textarea>/<select>` 를 `src/components/ui/` 밖에서 쓰는 건 지양 — 새 variant/size 가 필요하면 primitive 를 확장해서 별도 PR 로 (한 PR 한 변경 — §3.2).
...
- **Bracket border/shadow 게이트 (DS-6)**: memphis 그림자·두꺼운 보더의 임의값 `shadow-[Npx_Npx_…]` · `border-[Npx]` 하드코드는 `eslint.config.mjs` 의 `design-system/no-bracket-hardcodes` 룰이 차단 — 대체는 `shadow-memphis-{2xs,xs,sm,md,lg,2xl}`(+`-amore/-warning/-card`) · `border`/`border-2`(방향형 `border-{t,b,l,r}-2`). **hard(error, 머지 차단) 스코프는 `src/components/canvas/widgets/` 만** (DS-2 #969 가 baseline 0 확보한 유일 표면). 정확 일치 토큰 없는 잔존(색 오프셋 shadow, `border-[3px]` 등 9곳)은 `eslint-disable-next-line no-restricted-syntax -- 사유` 로 명시. ui/(잔여 `border-[2px]` 16곳 → `border-2` sweep 대기)·canvas/shell·피처 영역은 아직 미봉인 — radius/z-index 선례처럼 baseline 0 표면부터 단계적 확장.
...
- **디자인 시스템 SSOT 문서**: `docs/DESIGN_SYSTEM.md` (3-layer 토큰 + `data-theme` 톤 swap + primitive 카탈로그 + hardcoded 색 audit + 톤 swap 1-PR 가이드). 갱신은 `docs/DESIGN_SYSTEM_PENDING.md` inbox 패턴 — §5.4 의 PROJECT.md inbox 와 동일 원리. PR 머지 후 self-check 트리거 (새 primitive / 새 raw 토큰 / 새 톤 / hex sweep / variant 변경) 해당 시 한 줄 append, 5건 누적 시 묶음 갱신 PR. 자세한 룰은 DESIGN_SYSTEM.md §0.5.
...
| `docs/WRITING.md` | 카피·i18n SSOT (불변식·키 아키텍처·per-locale native 카피·4로케일 용어집·조판/포맷·한글 리터럴 가드 운영) |
...
- **2026-06-28** — 마이그 drift 사고: 14-digit 마이그 9건 머지됐으나 prod 미적용 → AI 동시통역 24h+ transcript 0건 (사용자 결제 데이터 손실 P0). jarvis 가 9건 push 로 응급 복구 (parking trick). 솔루션 E 도입: `sync.sh` 가 머지 감지 후 prod drift 자동 알림 + PR template Test plan 에 마이그 체크박스 + §3.8 표 / §7.5 자동 감지 fallback 갱신.
- **2026-07-13** — Auth 이메일을 Resend 커스텀 SMTP 로 전환 (§8.1 SSOT 신설). 2026-07-09 "인증메일이 갑자기 안 온다" 사고의 root cause = 내장 SMTP 시간당 ~2통 하드캡. 발신 `no-reply@research-canvas.io`, rate limit 30~100/시간 상향. 라이브 자격/값은 Supabase 대시보드(SMTP Settings + Rate Limits), repo 는 결정 SSOT + 검증 절차만 기록.
- **2026-07-13** — 가격 개편 착수: **₩500/cr(4배 인하) + 순마진 75% 불변식(`min-cr=⌈COGS/95⌉`, D1 CI 가드) + 하이브리드 스킴(크레딧 무할인·구독·무료 grant) + 통역 E1 가드레일**. 포지셔닝 A(대중 접근)로 전환. SSOT = [`docs/pricing-scheme.md`](docs/pricing-scheme.md) — 리프라이싱·구독·grant·통역 메터링 각 PR 의 단일 참조.
