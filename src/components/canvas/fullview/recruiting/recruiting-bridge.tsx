...
/* ────────────────────────────────────────────────────────────────────
   ...
   D1-A 개정(2026-07-25 사용자 결정 — 승인 게이트 폐기): 선택→보내기 =
   `recruiting_invitations` 생성 + **같은 요청 안에서 즉시 sched_candidates
   인제스트**(승인 대기 없음). 성공 시 호스트가 탭②(명단)로 전환해 방금 인원을
   노출한다. 연락처는 여전히 서버에서만 처리(응답 PII 는 이미 블랭킹돼 클라에
   없음 — 모달은 마스킹 값 ●●●● 만 표시). "승인 제거"지 "마스킹 제거"가 아님.
