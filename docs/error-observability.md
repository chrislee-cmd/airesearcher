...
### (3) 이메일 digest — Phase 2 (배포됨)
prod Vercel cron(`error-alert-digest`, `*/5`)이 `error_events` 에서
`alerted_at IS NULL AND resolved_at IS NULL AND severity='error'`(내부 `observability`
행 제외) 행을 모아 파운더에게 1 run = 1 이메일(count desc, 표시 cap 40 + "+N건").
보낸 행에 `alerted_at` 스탬프(dedup). `interview-failure-alert`(#1008) 를 대체 —
nodemailer + Gmail SMTP 재사용, 수신자 `ERROR_ALERT_EMAILS`(하위호환
`INTERVIEW_ALERT_EMAILS`→슈퍼어드민). #1008 의 stuck-pending / topline-error 감지는
status 컬럼만으로 못 잡으므로 `interview-extras-sweep` 로 `error_events` 에 이관
(widget-error-sweep cron 편입).
...
| **2** | 이메일 digest cron (prod): open 행 → 파운더 1 run = 1 이메일, `alerted_at` dedup. `interview-failure-alert`(#1008) 대체 + stuck-pending/topline 감지 이관 | **배포됨** |
