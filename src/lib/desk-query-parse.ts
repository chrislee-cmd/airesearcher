...
2. stat_terms (KOSIS/ECOS 통계 카탈로그용, 2-3개)
   ...
   - **하위 품목어 + 상위 카테고리어를 페어로 함께 낸다** — 좁은 품목이 KOSIS 0건이어도 상위어가 잡게 recall 을 이중화한다. 예: "이차전지"(하위) + "전지"(상위), "스킨케어"→"화장품"(상위)+"뷰티", "라면"→"면류"+"가공식품". 상위 카테고리 1개는 반드시 포함.
...
| "전기차 배터리 시장 얼마나 큰지" | 이차전지, 전지, 전기차 | LG에너지솔루션, 삼성SDI |
...
// (유저 무관) — PARSE_SYSTEM 이 바뀌면 버전(v1→v2 …)을 올린다.
export async function parseDeskQuery(
  ...
  const cacheKey = `desk-query-parse:v2:${locale}:${hashString(
  ...
  try {
    const { object } = await generateObject({
      ...
      temperature: 0.2,
      maxOutputTokens: 400,
      // 1→2: 오브젝트 파싱/일시 오류 복구 여지를 한 번 더 준다 (recall 회복).
      // 여전히 실패하면 catch 가 EMPTY 를 돌려 crawl 은 원 키워드로 진행 (throw X 보수성 유지).
      maxRetries: 2,
