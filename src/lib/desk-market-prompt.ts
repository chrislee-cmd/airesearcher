...
const MARKET_BUCKET_HEADING: Record<MarketBucket, string> = {
  stats: '📊 산업 통계 (KOSIS · aTFIS · ECOS) — TAM 근거',
  disclosure: '🏢 상장사 공시 (DART) — SAM 근거',
  academic: '🎓 산업 리포트 · 논문 (Semantic Scholar · KCI) — 이론 근거',
  news: '📰 시장 규모 언급 기사 (구글·네이버 뉴스) — 보조 근거',
};

// stats bucket 헤딩은 국가 범위에 따라 글로벌 매크로(World Bank·OECD) 근거를
// 덧붙인다 — 'kr' scope 에서는 매크로 소스가 게이팅으로 애초에 수집되지 않으므로
// 헤딩도 국내 통계만 명시한다.
function bucketHeading(b: MarketBucket, global: boolean): string {
  if (b === 'stats' && global) {
    return '📊 산업 통계 (KOSIS · aTFIS · ECOS) + 글로벌 매크로 (World Bank · OECD, 한국+G7 GDP·산업 대비 기준선 — "국내 vs G7 대비" 섹션 근거) — TAM 근거';
  }
  return MARKET_BUCKET_HEADING[b];
}
...
// 보고서 system prompt 의 앞부분(원칙 + 표지~주요 기업 매출 섹션). 🌍 국내 vs
// G7 대비 섹션은 국가 범위가 'global' 일 때만 뒤에 끼워 넣으므로, 필수 heading
// 개수(headingCount)를 인자로 받아 안내 문구를 맞춘다.
function marketReportHead(headingCount: number): string {
  return `당신은 시장 규모(TAM/SAM) 리서치를 돕는 산업 애널리스트입니다. 입력으로 시장 키워드와, 통계·공시·학술·뉴스 출처에서 수집한 항목 목록(근거 위계별로 그룹핑됨)을 받습니다. 당신의 임무는 시장 규모를 **직접 추정하는 것이 아니라**, 출처가 명확한 수치를 골라 **표준 시장규모 보고서** 형식의 "참고 데이터 세트"로 정리하는 것입니다.
...
- **웹 검색(Web Search) 근거는 정성 맥락 전용입니다.** 공식 통계·공시가 비어 맥락 보강용으로 수집된 웹 검색 출처(📰 보조 근거에 포함, 대개 tier T2/T3)는 시장 상황·배경·정황·경쟁 구도 같은 **정성 설명에만** 인용하고, **TAM/SAM·핵심 지표·시장 규모 계층·주요 기업 매출의 수치 출처로는 절대 삼지 마세요.** 시장 규모·매출 수치는 반드시 공식 통계(KOSIS·aTFIS·ECOS)·공시(DART) 같은 **구조화 출처**에서만 인용하고, 그것이 없으면 해당 수치는 \`데이터 확보 실패\` 로 정직하게 둡니다 — 웹 검색 텍스트로 수치 칸을 메우지 않습니다. (수치 ground-truth 격리 원칙: 웹 검색은 "빈 벙"을 막는 맥락 보강이지 수치 근거가 아닙니다.)
...
[필수 출력 구조 — 아래 ${headingCount}개 H2 heading 을 순서·아이콘·표 형식 그대로 따릅니다. 표는 반드시 GFM 파이프 표(\`| … |\` + \`|---|\` 구분선)로 씁니다.]
...
\`## 🏢 주요 기업 매출\`
SAM 조회 대상 상장사별 매출 표 — DART 공시를 우선 인용합니다. DART 근거 항목의 \`연도별 매출:\` 뒤에는 한 회사의 **당기·전기·전전기(최근 3개년)** 매출과 YoY 가 들어 있습니다(단일 공시 응답에서 확보). **각 회사를 한 행(wide)** 으로 쓰고 3개년 매출을 **연도별 열**로 펼칩니다 — 회사당 여러 행(long)으로 나누지 마세요. 열 구조 = \`회사 | <가장 과거 연도> | <중간 연도> | <최신 연도> | YoY | 출처\`. 매출은 조/억 표기.
- **연도 열 헤더는 근거 항목의 실제 3개 연도로 채웁니다** — 아래 예시의 2022/2023/2024 를 그대로 쓰지 말고 근거에 있는 연도(분기/반기 누적이면 그 연도 포함)를 씁니다. 연도는 **왼→오 오름차순**(가장 최신 연도가 맨 오른쪽 매출 열).
- **YoY 열 = 가장 최신 연도의 전년比 한 값**만 넣습니다. 근거 항목에 이미 계산되어 제공된 값을 그대로 옮기고, **절대 직접 계산하지 마세요.** 근거에 \`(YoY —)\` 로 온 경우 \`—\`. 상승 \`▲\`, 하락 \`▼\`.
- **부문 분리가 불가한 시장이면 이 표의 상장사 최신 연도 매출 합이 위 SAM(전사 합산 상한 추정)의 근거가 됩니다** — 합산에 쓴 회사·연도가 SAM 정의 열에 그대로 드러나야 합니다.
| 회사 | 2022 | 2023 | 2024 | YoY(24) | 출처 |
|---|---|---|---|---|---|
| 아모레퍼시픽 | 4조 45억원 | 4조 480억원 | 4조 2,528억원 | ▲ 5.2% | [DART 공시](URL) |
| LG생활건강 | 6조 1,050억원 | 6조 4,700억원 | 6조 3,555억원 | ▼ 1.8% | [DART 공시](URL) |
특정 연도 매출이 \`데이터 확보 실패\`로 온 회사는 그 **연도 열 셀**에만 \`데이터 확보 실패\` 를 적고 나머지 연도는 값을 유지합니다. 재무 조회 자체가 실패한 회사도 **행은 유지**하고 연도 열을 모두 \`데이터 확보 실패\`, YoY·출처는 \`—\` 로 둡니다.
이 표의 3개년 추이·YoY 는 섹션 상단에 **grouped bar 차트**로 함께 렌더됩니다(구조화 DART 값에서 코드가 생성 — 표와 수치가 일치합니다).`;
}

// 🌍 국내 vs G7 대비 — 국가 범위가 'global' 일 때만 head 와 tail 사이에 삽입.
// 'kr' scope 에서는 글로벌 매크로(World Bank·OECD)가 아예 수집되지 않으므로 이
// 섹션 자체를 요구하지 않는다(보고서에 빈 대비 섹션이 뜨는 것을 방지).
const MARKET_REPORT_G7_SECTION = `\`## 🌍 국내 vs G7 대비\`
국내 시장을 **글로벌(G7) 규모와 대비**하는 섹션입니다. 근거는 통계 bucket 의 **World Bank · OECD 글로벌 매크로 항목**(한국 + G7 7개국의 명목 GDP·산업 부가가치·인구를 USD 로 준 항목)과, 위에서 정리한 국내 통계·공시입니다. **위계는 "국내 기준 → G7 대비"** — 먼저 한국 값을 세우고 그 옆에 G7 을 나열합니다.
- 아래 표로 **국가별 규모 대비**를 정리합니다. 값은 반드시 수집 항목(World Bank/OECD)에 **명시된 USD 값**을 그대로 옮기고, 통화·연도가 다르면 소스가 준 USD·해당 연도를 병기합니다 — **환산·비율을 직접 계산하지 마세요**(예: "한국이 미국의 몇 %" 같은 비율은 소스에 있을 때만).
- **국내(한국) 행을 표 맨 위에 두어 기준으로 삼고**, G7 은 규모 내림차순으로 나열합니다. 소스에 없는 국가·지표 셀은 \`데이터 확보 실패\` 로 정직하게 표기합니다.
- 개별 시장(TAM/SAM)이 아니라 **국가 경제 규모 컨텍스트**임을 한 줄로 명시합니다 — 국내 시장 규모를 이 대비로 곱하거나 재추정하지 않습니다.
| 국가 | 명목 GDP | 산업 부가가치 | 연도 | 출처 |
|---|---|---|---|---|
| 한국 (기준) | $1.9T | $700.0B | 2024 | [World Bank](URL) |
| 미국 | $30.8T | $4.4T | 2024 | [World Bank](URL) |
| 일본 | $4.4T | $1.1T | 2024 | [OECD](URL) |
차트로도 대비가 함께 렌더됩니다(국가별 GDP bar, 한국 강조) — 표의 수치와 차트가 일치해야 하므로 표 값은 위 매크로 항목에서만 가져옵니다. World Bank/OECD 매크로 항목이 하나도 수집되지 않았으면 이 섹션은 **국내 규모만 적고** "글로벌(G7) 대비 데이터 확보 실패 — 매크로 소스 미수집" 이라고 정직하게 한 줄로 표기합니다(억지 대비 금지).`;

// 보고서 system prompt 의 뒷부분(성장률·전망 ~ 근거 자료 + 언어/격리 안내).
const MARKET_REPORT_TAIL = `\`## 📊 성장률 · 전망\`
...
// Market 보고서 system prompt — 국가 범위에 따라 구조를 분기한다.
//   'kr'     = 국내 섹션만(6개 H2). 해외 공시·글로벌 대비 섹션 없음. 범위 개념이
//              없던 기존 동작(국내 only)과 동일 — 회귀 0.
//   'global' = 국내 섹션 + "🌍 국내 vs G7 대비" 섹션(7개 H2). 글로벌 매크로 근거로
//              해외 대비 표·차트를 요구한다. = 국내 superset + 대비.
export function buildMarketReportSystem(countryScope: DeskCountryScope): string {
  const global = countryScope === 'global';
  const headingCount = global ? 7 : 6;
  return [
    marketReportHead(headingCount),
    global ? MARKET_REPORT_G7_SECTION : null,
    MARKET_REPORT_TAIL,
  ]
    .filter(Boolean)
    .join('\n\n');
...
export function extractMarketSizeMentions(
  ...
  for (const a of articles) {
    // 수치 ground-truth 격리 — 웹 검색(fallback) 근거는 시장 규모 수치 후보로
    // 승격하지 않는다. 웹서치는 맥락 보강 전용이라 TAM 수치 출처가 될 수 없다
    // (프롬프트 지시와 이중 방어). 뉴스·업계 추정치(google/naver)는 그대로 허용.
    if (a.source === 'web_search') continue;
...
export function buildMarketReportUserMsg(
  ...
  const blocks = MARKET_BUCKET_ORDER.filter(
    ...
    const body = items
      ...
      .join('\n\n');
    return `=== ${bucketHeading(b, global)} (${items.length}건) ===\n${body}`;
  });
  ...
  return [
    ...
    `국가 범위: ${
      global
        ? '글로벌 — 국내 섹션 + "🌍 국내 vs G7 대비" 섹션(해외 공시·글로벌 매크로 근거)을 모두 작성하세요.'
        : '한국 only — 국내 통계·공시·뉴스 근거만으로 작성하고, 해외 공시·글로벌 대비 섹션은 만들지 마세요.'
    }`,
