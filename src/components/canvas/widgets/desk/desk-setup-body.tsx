...
/* ────────────────────────────────────────────────────────────────────
   DeskSetupBody — 데스크 리서치 V2 카드 본문 (CD 파일럿 #2, fresh build).

   WIDGET-SHELL §AUTHORITY §D: CD `.dc.html` 대로 신규 작성한 프레젠테이션.
   옛 `desk-card-body.tsx` / `ControlBoardPanel`(control-panel) 재사용 X —
   ...
   구성 (WIDGET-SHELL §S1 조립):
   - setup: 4스텝 아코디언 (all-open ↔ collapsed 요약)
   - started/done: in-place Handoff (리포트는 fullview=PR2)
   - §3 상태 배너 전부 (error·timeout·stuck·fallback·raw-dump·done-empty·
     cancelled·skipped)
   - footer: footNote(좌) + CTA(우) 한 줄, border-top (§S1)
