...
export async function runTopline(
  ...
  try {
    ...
    const stream = streamObject({
      ...
      prompt: `위는 이 프로젝트의 응답자 ${docs.length}명을 한 명도 빠짐없이 순회해 뽑은 주제·인용 추출 + 서버가 코드로 카운트한 **사전집계 표**입니다. 이를 종합해 깊이 있는 탑라인 보고서를 블록 배열로 작성하세요. **맨 첫 블록은 executive_summary(리치 요약 문단 4~6문장 + 핵심 포인트 3~5)** 로 시작하고, 이어서 응답자 프로필 → 핵심 요약 → 코퍼스에서 도출한 주제별 섹션들(근거가 허용하는 만큼 많이) → 교차분석 인사이트 → 시사점 순으로, 각 섹션을 subheading + paragraph(불릿 병행) 로 2단 계층으로 전개하고, 주장 뒤에 quote 를 문맥 중간에 삽입하며, table + chart/pie 를 유기적으로 배치합니다. **표의 집계 수치는 위 "사전집계 표"에서 그대로 인용**하고(직접 세거나 추정 금지), 사전집계에 없는 서술 수치는 만들지 마세요. 각 테마 섹션은 사전집계에 해당 데이터가 있으면 **정량 표를 1개 이상** 포함하고, 세그먼트 차이(%p)·"전체 평균의 함정"을 살립니다. 모든 사실 블록에 근거 chunk_id 를 답니다. 이전보다 훨씬 길고 상세하게.`,
      temperature: 0.3,
      // 긴 보고서(다수 섹션 + 서브헤더 + 사전집계 기반 표 다수)라 출력 예산을
      // 대폭 상향해 잘림(finishReason='length')을 방지한다. Opus 4.8 는 큰 출력을
      // 지원한다. 뎁스 상향(카드 603)으로 32k→40k 상향.
