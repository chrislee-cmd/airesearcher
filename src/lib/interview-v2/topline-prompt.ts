...
### 객관식(닫힌 문항) 전수집계 ★최우선 — 최대 실패 모드 (§2-4·§4)
- **탐지 휴리스틱**: 근거 블록에서 답변이 (a) 짧고(대략 70자 미만) (b) 서로 다른 값의 종류가 적은(보기 개수 수준) 문항은 **객관식(닫힌 문항) 후보**입니다. 이런 문항은 언급한 응답자만 세지 말고 **응답자 전원(분모 N)을 기준으로 전수 집계**합니다.
- **응답 텍스트만 집계**: 집계 대상은 각 응답자의 **응답부만** 입니다 — 문항·보기 목록 텍스트까지 함께 매칭하면 특정 항목이 100% 로 잡히는 가짜 집계가 됩니다(§7 정규식 오염). 보기 목록 문자열은 집계에서 제외합니다.
- **세그먼트별 보기가 다른 문항**: 그룹마다 보기 문구가 다르게 설계된 문항(예: "다른 스킨케어처럼 매일" vs "아이 루틴으로 매일")을 그냥 합치면 0%·48% 같은 **가짜 극단값**이 나옵니다. 표에 \`*\` 표시 + 해당 그룹 기준 수치 병기 + **합산 행을 별도로** 둡니다.
- ⚠️ **한계(정직 고지)**: 여기서는 데이터 검사 스크립트 없이 **위 휴리스틱으로만** 객관식을 탐지하므로 짧은 객관식 문항을 통째로 놓칠 위험이 완전히 사라지진 않습니다. 짧고 값 종류가 적은 문항이 보이면 **먼저 객관식일 가능성을 의심**해 전수 집계 대상에 넣으세요. (객관식 근거 자체가 없는 코퍼스면 이 규칙은 무발화 — 지어내지 않습니다.)

### 집계 전 정규화 (§5-4)
- **브랜드명 별칭 통합**: 음성 자동 전사는 브랜드명을 심하게 왜곡해 하나의 브랜드가 여러 표기로 흩어집니다(예: Eucerin → Usurin/Zucerin/uterine). 침투율·점유를 집계하기 전에 명백한 전사 변형을 **같은 브랜드로 통합**해 수치가 쪼개지지 않게 합니다.
- **응답자 ID 충돌**: 세그먼트 간 파일 번호가 겹칠 수 있으므로, 본문의 "응답자 N" 표기가 인용 출처(파일)와 어긋나지 않도록 그룹 접두사나 통합 번호로 일관되게 지칭합니다.
...
### 제출 전 완결성 역점검 (필수)
보고서를 마무리하기 전에, 아래 발견 유형이 데이터에 있는지 **역으로 확인**하고 있으면 반드시 반영하세요:
- 한 그룹이 특정 보기에서 **0%** · 그룹 간 **15%p 이상** 격차 · 카테고리 밖 제품이 높은 점유 · **선호와 실제 사용의 갭** · 예상 채널이 한 자릿수 · 응답자 본인이 겪는 고통(구매자 ≠ 사용자).
- 이 유형이 **하나도 나오지 않으면 집계가 부실**하다는 신호입니다 — 근거를 다시 훑어 놓친 집계가 없는지 점검하세요. (근거에 실제로 없으면 지어내지 말고 그대로 둡니다.)
...
그 사이(핵심 요약 다음 ~ 교차분석 전)에는 **코퍼스에서 실제로 도출되는 주제별 섹션을 근거가 허용하는 만큼 많이**(상한을 두지 말고, 근거가 있는 모든 주제를) 만듭니다. 섹션 이름은 데이터에 맞게 정하세요(예: 사용 행태 / 구매 채널 / 제품 선택 기준 / 페인포인트 / 정보 탐색·신뢰 / 브랜드·라벨 인식 등 — 코퍼스에 근거가 있는 주제만). 각 주제 섹션은 subheading 여러 개 + paragraph + 적절한 아티팩트로 깊이 있게 전개하고, **해당 주제에 대한 사전집계 표(전체/세그먼트)가 있으면 정량 표를 1개 이상 반드시 포함**합니다(사전집계에 데이터가 없는 주제는 서술·인용 위주로).
...
- **표·분포 수치는 위 "사전집계 표"를 최우선 SSOT 로 그대로 인용**하세요(직접 다시 세지 말 것). 사전집계 표에 이미 있는 문항·세그먼트 값은 재계산·반올림·추정하지 않습니다.
- 사전집계 표에 **없는** 정성 빈도(예: 특정 페인포인트를 언급한 응답자 수)만 제공된 N명 추출을 훑어 **직접 카운트**해 보완합니다(추정·반올림 금지). 분모 N = 제공된 응답자 총수.
- chart/pie/table 의 수치도 사전집계 또는 이 전수 카운트에서 산출합니다(예: 주제별 언급 응답자 수). 근거에서 셀 수 없는 수치는 만들지 마세요.
