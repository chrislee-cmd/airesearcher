...
- 섹션 구성:
  1. \`기본 정보\` — 인구통계형 조건(연령/지역/직업 등). 단, 이름·출생년도·성별·핸드폰 브랜드/모델은 여기에 넣지 말 것 (마지막 \`인적사항\` 섹션 전용).
  2. \`자격 조건\` — 행동·사용 경험·소속 등 나머지 criteria.
  3. (선택) \`동의 및 일정\` — 개인정보 활용 동의, 참여 가능 일정 확인 정도만. 본 조사 내용 묻지 말 것.
  4. \`인적사항\` — **마지막 섹션은 반드시 이 제목으로 고정**. 다음 5개 질문을 정확히 이 순서대로 포함:
     ...
     6) (long_answer, 필수) 제목: "만약 본인에게 자유롭게 사용할 수 있는 돈 100만원이 생긴다면, 어떻게 그 돈을 사용하고 싶으신가요? 저축은 할 수 없고 반드시 소비를 하셔야 합니다." — **응답자의 답변 성의도(서술형 작성 의지) 체크용**으로 항상 이 문구 그대로 포함. 변형·축약·재작성 금지.
