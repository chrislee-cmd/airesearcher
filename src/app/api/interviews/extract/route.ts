...
3) 한국어로 작성. 출력은 정의된 JSON 스키마만, 그 외 텍스트 금지.${ISOLATION_NOTICE}`;
...
export async function POST(request: Request) {
  ...
  const markdownSlice = markdown.slice(0, 200000);
  const markdownSan = await sanitizeUserInput(markdownSlice, 'interview_markdown', {
    endpoint: '/api/interviews/extract',
    user_id: user.id,
    org_id: org.org_id,
    actor_email: user.email ?? null,
    input_length: markdownSlice.length,
    input_label: 'interview_markdown',
  });
