      ...
      const [textPipeline, rolesRes, diarRes, summaryRes] = await Promise.all([
        (async () => {
          ...
          const termNormalize = await normalizeTermsInTranscript(
            ...
            console.warn(
              '[transcripts/webhook/elevenlabs] term-normalize pass failed',
              e,
            );
            return null;
          });
        ...
        // Q&A 문맥 diarization — single-speaker + 60s+ 만 호출. 통역사 1인
        // 인터뷰 시나리오 cover. monologue 면 자동 폐기.
        shouldDiarize
          ? classifyQaDiarization(mergedWords, job.filename, formatted.duration).catch(
              (e) => {
                console.warn(
                  '[transcripts/webhook/elevenlabs] diarization pass failed',
                  e,
                );
                return null;
              },
        ...
        shouldSummarize
          ? summarizeMeeting(formatted.markdown, job.filename).catch((e) => {
              console.warn(
                '[transcripts/webhook/elevenlabs] meeting-summary pass failed',
                e,
              );
              return null;
            })
