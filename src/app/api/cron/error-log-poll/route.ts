...
// 반환: { message, timestamp } 배열. 실패/미설정 시 throw → 상위에서 self-log.
async function fetchPostgresErrorLogs(
  ...
  // Supabase 로그(Logflare/BigQuery 백엔드) 표준 스키마.
  const sql = `
    select
  ...
  const json = (await res.json()) as { result?: { message?: string; timestamp?: string | number }[] };
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: true, polled: true, ingested: logs.length, window: { start: startIso, end: nowIso } });
}
