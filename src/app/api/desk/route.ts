...
- 섹션 헤더 앞에는 의미가 통하는 이모지를 1개씩 붙입니다 (예: 🧾 TL;DR, 📈 트렌드, 📰 채널별, 🔎 주목할 항목, ⚠️ 한계). 본문 내 이모지는 절제해서 사용.
- 모든 링크는 반드시 \`[제목](URL)\` 형식의 markdown 링크. 절대 raw URL 노출 금지.
- 강조는 **굵게**, 인용은 \`> 인용문\` 형식 가능.

다음 섹션을 포함하세요:
1. \`# 🗞 데스크 리서치 요약\` — 키워드와 기간을 표지에 표기
2. \`## 🧾 핵심 요약 (TL;DR)\` — 5개 이내 불릿, 각 항목은 한 문장
3. \`## 📈 주요 흐름 / 트렌드\` — 반복 등장 토픽, 상반된 시각, 톤. 2~4개 단락 또는 불릿
4. \`## 📰 채널별 관찰\` — 네이버(뉴스/블로그/카페/지식인), 카카오·다음, 유튜브, 글로벌(구글뉴스/HN/Reddit) 중 데이터가 있는 채널만. 채널 이름은 \`### 네이버 뉴스\` 같이 소제목으로 시작, 그 아래 1~2단락
5. \`## 🔎 주목할 항목\` — 시그널이 강한 6~10개. 한 줄당 \`- [제목](URL) — 한 줄 요약 (출처 · 날짜)\` 형식
6. \`## ⚠️ 한계 / 추가 조사 제안\` — 데이터 부족 사유, 후속 리서치 아이디어 2~4개

사실을 임의로 추가하지 말고 제공된 자료에 근거. 자료에 없는 수치·날짜·이름은 만들어내지 않습니다.`;
