...
- **근거가 없으면 빈칸을 임의 수치로 채우지 말고 정확히 \`데이터 확보 실패\` 라고 적습니다** (표 셀·KPI 값 모두 동일 문구). 필요하면 괄호로 짧은 사유 — 예: \`데이터 확보 실패 (근거 통계 없음)\`. YoY·출처처럼 값이 없는 부수 열은 \`—\` 로 둡니다.
...
\`## 📈 핵심 지표\`
아래 표 한 개 — TAM·SAM·CAGR 3행을 원칙으로 하되 근거 있는 지표만 넣습니다. 값이 없으면 \`데이터 확보 실패\`.
| 지표 | 값 | 대상 | 기간 | 출처 |
|---|---|---|---|---|
