...
## 5. IA 개정 각주 — 3탭 → 2탭 재구성 (사용자 결정 2026-07-26, #578·#579)
> **의도된 이탈.** 이 CD 번들(.dc.html + BUILD-SPEC)은 **3탭 IA(N1 응답 · N2 명단 · N3 일정)** 기준으로 작화됐으나, 사용자 결정으로 제품은 **2탭으로 재구성**됐다. CD 프레임과 어긋나는 아래 지점은 conformance 에서 버그로 오인하지 말 것 — **후속 CD 작화 요청 대상**이다.

**재구성 맵:**
- **탭②(명단, N2) 제거.** 명단 조율(로스터/그룹/후보 풀)은 **#578 로 탭③(일정)이 흡수**(전체/확정 토글 단일 리스트). 유입 3종(응답연동 카운트🔒 · CSV/Excel 업로드 · Google Sheets 연동+연동됨 카드)은 **#579 로 탭①(응답)으로 이관** — 폼 셀렉터/뷰 토글 툴바 아래 접이식 "명단 소스" 밴드(`journey-intake-band.tsx`).
- **최종 2탭:** ① 응답(응답 확인 + 명단 유입 창구) · ② 일정(명단 조율 + 캘린더/채팅/슬롯). 라벨 순번은 ①②로 재배치(옛 "③ 일정" → "② 일정").
- **CD 프레임 갭(후속 작화 필요):** (a) N2 명단 탭의 3-up 소스 카드가 **단독 탭이 아닌 응답 탭 내 접이식 밴드**로 재배치된 레이아웃, (b) 밴드의 접힘/펼침·연동됨 요약 1줄 상태, (c) 2탭 폴더 내비의 순번/count pill 재배치. 현재는 기존 N2 소스 카드 chrome(memphis border-2·shadow-sm·success-bg)을 그대로 이식해 밴드로 감쌌다.
- **유지된 계약:** 업로드 앵커(inbox batch)·서버 PII 마스킹·OAuth 바운스(share=1)·연동됨 카드(R9)·소스 프로비넌스 컬럼(§1-7)·568 폭 봉쇄 — 전부 이관 후에도 불변.
