...
| 10 | 개인정보/분석 | PostHog가 consent gate 없이 무조건 캡처 | 쿠키 배너(PR-SEC8) 약속과 불일치 — autocapture + session recording이 동의 전에 전송됨. EU 트래픽에서 GDPR 리스크 | 사용자 결정(2026-07-03): 초기엔 full 캡처 우선, 규모 커지면 재점검. Mixpanel처럼 `PostHogProvider`를 `useConsent().analytics`에 gate (init 후 revoke 시 `opt_out_capturing` + `reset`). `initPostHog`는 이미 분리돼 있어 provider 3줄 수정으로 전환 가능 |
