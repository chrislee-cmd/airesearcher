...
전형 시나리오:
- prod 도메인 신규 발급 (`research-canvas.io`) 후 `GOOGLE_REDIRECT_URI` env 만 갱신, Google Cloud Console 의 OAuth client 업데이트 누락
...
|---|---|---|
| Google Cloud Console client 이름 | `Research-Canvas Supabase Auth` (v2) | `ai researcher web` |
...
| 환경 | URI |
|---|---|
| Production | `https://research-canvas.io/api/share/google/callback` |
...
| 환경 | Origin |
|---|---|
| Production | `https://research-canvas.io` |
| Preview | `https://airesearcher-woad.vercel.app` |
| Local | `http://localhost:3000` |

### 3.3 Scope / consent screen

Scope 변경은 코드 (`src/lib/google-oauth.ts` 의 `SHARE_SCOPES`) 에서만. consent screen 은 OAuth consent screen 탭에서 `Research-Canvas` 와 동일 설정 사용 (project 단위 공유).
...
| `GOOGLE_REDIRECT_URI` | `https://research-canvas.io/api/share/google/callback` | `https://airesearcher-woad.vercel.app/api/share/google/callback` | `http://localhost:3000/api/share/google/callback` |
...
- [ ] **시크릿 창** 에서 `https://research-canvas.io` 로그인 → 전사록 위젯 → 산출물 → 공유 → 워드 (Google Docs)
...
만약 client 자체를 교체하고 싶다면 (예: 새 `Research-Canvas Drive` branded client 신규):
