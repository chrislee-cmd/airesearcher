...
| sync.sh 마이그 drift 감지 | sync.sh (jarvis 실행, 머지 감지 후) | **soft** | `supabase migration list --linked` 비교 → prod 미적용 마이그 발견 시 한 줄 알림. 적용 자체는 수동 (CLI 없으면 silent skip — §7.5 참고) |
...
**자동 감지 (2026-06-28 사고 후 도입)**: jarvis 의 `sync.sh` 가 머지 감지 직후 `supabase migration list --linked` 로 prod-미적용 마이그 발견 시 한 줄 알림 (`⚠️  X건 마이그 prod 미적용 — supabase db push 필요`). 알림 보이면 즉시 `supabase db push --linked --yes` 실행. supabase CLI 없는 환경에서는 silent skip — 적용 자체는 여전히 인간 책임 (soft 알림, sync.sh 막지 않음).
