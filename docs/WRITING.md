...
### 하드코딩 → 토큰 승격 루프 (writing-system 이 성장하는 방식)
가드가 유저-facing 하드코딩을 잡으면 **다시 하드코딩으로 번역하거나 `Intl.*` 로
우회하지 말고, messages 토큰으로 승격**한다. 그래야 writing-system(messages)이
UI 카피의 단일 SSOT 로 **단조 성장**하고, 어떤 문구든 코드 수정 없이 교체·오버라이드
가능해진다. 한 방향 루프:

1. **식별** — `pnpm check:korean` 가 유저-facing 한글 리터럴을 파일별로 플래그.
2. **승격** — 그 리터럴을 messages 키로 이관(en 원문 + ko). 위치·색이 아니라
   의미 기반 키(§2). *다시 하드코딩 X, Intl 런타임 우회 X.*
3. **자동 채움** — `pnpm i18n:seed` 가 ja/th 를 백필(용어집 §5 + 2-pass + 게이트).
   토큰 1개 추가의 한계비용이 ≈0 이라 승격이 싸다 — 이게 루프를 "지속" 가능케 함.
4. **락** — 4로케일 exact parity 게이트(`check-i18n.ts`)가 그 키의 전 로케일 존재를
   영구 보장. baseline 은 그만큼 조여지고(단조 감소), 재유입은 가드가 차단.

표면별로 반복하면 유저-facing baseline 이 0 으로 수렴 = writing-system 이 UI 카피를
전부 흡수한 종료 상태. 언어명 드롭다운(`Languages` 네임스페이스)이 이 루프의 레퍼런스
사례: `한국어` 하드코딩 → `Languages.ko` 토큰 → seed 가 韓国語/เกาหลี 자동 생성.
남은 유저-facing 언어 셀렉터(topline 출력언어 등)도 같은 토큰을 채택하면 된다.
