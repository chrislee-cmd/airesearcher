...
export function buildInsightsClustersSystem(lang?: OutputLang): string {
  return `당신은 인사이트 분석가입니다. 한 인터뷰 분석의 인용구 묶음을 받아, 의미적으로 가까운 quote 들을 3~7개의 클러스터로 묶으세요. 결과는 정의된 JSON 스키마만, 그 외 텍스트 금지.
...
- 출력 언어: label/insight 등 **LLM 이 서술하는 텍스트는 ${lang ? outputLangLabel(lang) : '입력 언어'}(으)로** 작성합니다. lang 미지정이면 입력 언어를 따릅니다(한국어 데이터면 한국어). **quote 원문·인용은 항상 원래 언어 그대로** 둡니다.${ISOLATION_NOTICE}`;
}
