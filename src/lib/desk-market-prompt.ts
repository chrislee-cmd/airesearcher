...
\`## 🏢 주요 기업 매출\`
SAM 조회 대상 상장사별 매출 **시계열** 표 — DART 공시를 우선 인용합니다. DART 근거 항목의 \`연도별 매출:\` 뒤에는 한 회사의 **당기·전기·전전기(최근 3개년)** 매출과 YoY 가 들어 있습니다(단일 공시 응답에서 확보). **각 회사를 연도별로 최대 3행**(최신 연도가 위)으로 펼쳐, 3개년 추이가 한눈에 보이게 합니다. 매출은 조/억 표기, YoY 는 ▲/▼.
- **YoY 는 근거 항목에 이미 계산되어 제공된 값을 그대로 옮기고, 절대 직접 계산하지 마세요.** 근거에 \`(YoY —)\` 로 온 기간(가장 과거 연도·결측·기준 상이 등)은 YoY 열에 \`—\` 로 둡니다.
- **부문 분리가 불가한 시장이면 이 표의 상장사 전사 매출(가장 최신 연도) 합이 위 SAM(전사 합산 상한 추정)의 근거가 됩니다** — 합산에 쓴 회사·연도가 SAM 정의 열에 그대로 드러나야 합니다.
| 회사 | 연도 | 매출(연결) | YoY | 출처 |
|---|---|---|---|---|
| 아모레퍼시픽 | 2024 | 4조 2,528억원 | ▲ 5.2% | [DART 공시](URL) |
| 아모레퍼시픽 | 2023 | 4조 480억원 | ▲ 1.1% | [DART 공시](URL) |
| 아모레퍼시픽 | 2022 | 4조 45억원 | — | [DART 공시](URL) |
| LG생활건강 | 2024 | 6조 3,555억원 | ▼ 1.8% | [DART 공시](URL) |
특정 연도 값이 \`데이터 확보 실패\`로 온 회사는 그 연도 행의 매출 열에 \`데이터 확보 실패\`, YoY·출처 열은 \`—\`. 재무 조회 자체가 실패한 회사도 **행은 유지**하고 매출 열에 \`데이터 확보 실패\`, 나머지는 \`—\` 로 둡니다.
