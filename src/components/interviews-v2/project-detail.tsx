...
export function ProjectDetail({
  ...
  const { documents, isLoading } = useInterviewV2Documents(projectId);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {/* 서브헤더 — 좌: 뒤로 + 설정/업로드, 우: 검색. 설정/업로드/검색은
          다음 spec 에서 wire (지금은 placeholder disabled). */}
      ...
      {/* 본문 — 좌(파일 list) 5/12 + 우(검색 chat, 다음 spec). */}
