...
//
// 데이터 소스 전환 (2026-07-04): 예전엔 이 패널이 자체적으로
//   GET /api/recruiting/google/forms/[id]/distribution
// 를 호출해 **서버가 원본 응답으로 집계한** crosstab 을 받았다. 그래서 사용자가
// 필터(셀/질문)를 걸어도 분포 수치는 원본 그대로였다 (spreadsheet 만 필터되고
// 분포는 안 변하는 P0 sync bug). 이제 호스트(recruiting-card)가 spreadsheet 이
// 로드한 응답(columns + rows)을 그대로 내려주고, 이 패널이 **필터 적용 후**
// 클라이언트에서 buildDistributionTable 로 재계산한다 — spreadsheet 와 완전히
// 동일한 rows 를 쓰므로 두 뷰가 절대 어긋나지 않는다.
//
// 안전성: 성별/연령 값은 PII 마스킹 대상이 아니고(마스킹은 이름/전화만),
// responses/distribution 두 엔드포인트가 동일하게 consent 필터를 적용하므로
// 클라이언트 재계산 결과는 옛 서버 집계와 (필터 없을 때) 정확히 일치한다.
//
...
export function RecruitingDistributionPanel({
  ...
  // 필터 적용 후 rows → crosstab 재계산. spreadsheet 이 화면에 그리는 것과
  // **같은** matchesFilter 로 좁히므로 분포 수치와 표가 항상 일치한다.
  const filteredRows = useMemo(
    () => rows.filter((r) => matchesFilter(r, filter, columns, nowYear)),
    [rows, filter, columns, nowYear],
  );
  const table = useMemo(
    () => buildDistributionTable(columns, filteredRows, { nowYear }),
    [columns, filteredRows, nowYear],
  );
...
function PanelBody({
  ...
  if (table.grandTotal === 0) {
    // 필터 때문에 비었는지, 원래 응답이 없는지 구분.
    return hasActiveFilter(filter) ? (
      <EmptyState
        tone="subtle"
        title="조건에 맞는 응답이 없습니다"
        description="선택한 셀·질문 조건에 해당하는 응답이 없어요. 위 필터를 초기화하면 전체 분포가 다시 표시됩니다."
      />
    ) : (
      <EmptyState
        tone="subtle"
